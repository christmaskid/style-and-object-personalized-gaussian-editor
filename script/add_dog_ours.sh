...
python launch.py --config configs/add.yaml \
    ...
    system.inpaint_prompt="a dog on the bench" \
    system.refine_prompt="make it a dog" \
    system.cache_overwrite=False system.cache_dir="add_dog"  \
    trainer.max_steps=1  \
    system.loggers.wandb.enable=true \
    system.loggers.wandb.name="add_dog"