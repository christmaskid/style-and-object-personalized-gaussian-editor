...
class AddGuidance:
    def __init__(self, guidance, latents, gaussian, origin_frames, text_prompt, per_editing_step, edit_begin_step,
        ...
        self.prompt_utils = StableDiffusionPromptProcessor(
            {
                "pretrained_model_name_or_path": "runwayml/stable-diffusion-v1-5" 
                        if custom_diffusion_model is None else custom_diffusion_model,
                "prompt": text_prompt,
            }
        )
        self.step = 0
    ...
    def __call__(self, rendering, view_index, step):
        ...
        if view_index not in self.edit_frames or (
                ...
                and self.step % self.per_editing_step == 0
        ):
